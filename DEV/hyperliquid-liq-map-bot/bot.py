#!/usr/bin/env python3
"""
HL-MOMENTUM-BOT — Hyperliquid Futures Trend + Momentum Bot (sem liquidações externas)

Versão simplificada do antigo HL-LIQ-MAP-BOT:
  - Remove completamente dependência de CoinGlass/Coinalyze para evitar erros de API.
  - Fica um bot "institucional" clássico baseado em:
      * EMA 21/55/233 + VWAP (tendência e contexto)
      * RSI(14), MACD(12,26,9), StochRSI(14,3,3)
      * ATR(14) para SL/TP e dimensionamento de posição.

Estratégia em 15m (configurável em config.json):
  - Só LONG a favor da tendência bullish (preço acima do VWAP, EMAs alinhadas).
  - Entrada em pullbacks "limpos" com confluência dos osciladores.
  - SL/TP calculados por ATR com R:R configurável.

Hyperliquid SDK docs: https://hyperliquid.gitbook.io/hyperliquid-docs/for-developers/api/exchange-endpoint
"""
...
# ─────────────────────────────
#  SIGNAL ENGINE (Tendência + Momentum)
# ─────────────────────────────
